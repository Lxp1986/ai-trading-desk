...
- **多空双向（USDT 本位永续）**：buy 信号开多 / sell 信号开空（SWAP 合约，cross 保证金，双向持仓模式）；现货保留账户资产；
- **动态杠杆引擎**（专业交易员多因子）：杠杆 = 2x 基准 × 波动率 × 顺势 × 信号强度 × 策略胜率 × 防守，clamp [1, 5]x——低波动+顺势+高置信+高胜率 → 5x；高波动/逆势/回撤/连亏 → 1x；风险预算红线（总权益 1%）不变；
- **仓位与总资金匹配**：资金基准 = OKX 账户总权益（~$80k 虚拟）；单笔风险预算 = 总权益 × 1%；单笔名义上限 = min(总权益 × 20%, 可用 USDT × 30%)；最多 3 持仓；ETH 双向交易已授权（需买可买、缺 USDT 可卖已有 ETH）；
- **学习闭环**：平仓盈亏按策略+方向归因 → 策略权重自适应 + 方向胜率反馈到动态杠杆；盘后复盘改进 → 次日落地；
